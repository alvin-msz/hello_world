...
# VT 向公版 tiny/small 对齐（Single* 单帧；HAT 的 MSDeformAttn3D 不支持多尺度 FPN）
# 参考: https://github.com/fundamentalvision/BEVFormer
#   公版 tiny/small: encoder=3, pillar=4, SCA.pts=8, 单尺度 C5
#   本配置：encoder=3 + 密采样 + 加密 virtual BEV；保持单尺度 stride32
task_name = "bevformer_henet_camera_multitask_nuscenes_vt_heavy"
batch_size_per_gpu = 2
device_ids = [4, 5, 6, 7]

ckpt_dir = f"./tmp_models/{task_name}"

# Occ3D LMDB paths — same as flashocc_henet_lss_occ3d_nuscenes.py for shared data prep.
...
_dim_ = 256
_pos_dim_ = _dim_ // 2
# HAT HorizonMultiScaleDeformableAttention3D 仅单尺度可用（多尺度会 16 vs 64 shape mismatch）
_num_levels_ = 1
bev_h_ = 128
bev_w_ = 128
input_size = (512, 960)
# stride32 feat: [W, H] = [960/32, 512/32]
_vt_feats_size_ = [[input_size[1] // 32, input_size[0] // 32]]  # [[30, 16]]
map_size = (15, 30, 0.15)
use_lidar2img = True
with_ego_occ = False

# 公版 SCA 会看全部相机；HAT 用 max_camoverlap 做 rebatch，适当加大
max_camoverlap_num = 4
bev_sparse_rate = 0.75
# 对齐公版 tiny/small：encoder=3, pillar=4, SCA.pts=8（须能被 pillar 整除）
vt_num_layers = 3
vt_num_points_in_pillar = 4
vt_sa_num_points = 8
vt_ca_num_points = 8
# virtual BEV 加密：原 64x80→接近全分辨率
vt_virtual_bev_h = int(bev_sparse_rate * bev_h_)  # 96
vt_virtual_bev_w = bev_w_  # 128
...
camera_network = dict(
    ...
    view_transformer=dict(
        ...
        bev_w=bev_w_,
        pc_range=point_cloud_range,
        num_points_in_pillar=vt_num_points_in_pillar,
        embed_dims=_dim_,
        queue_length=1,  # 单帧；时序需 BevFormerViewTransformer + TemporalSA + num_seq>1
        in_indices=(-1,),
        single_bev=True,
        use_lidar2img=use_lidar2img,
        max_camoverlap_num=max_camoverlap_num,
        virtual_bev_h=vt_virtual_bev_h,
        virtual_bev_w=vt_virtual_bev_w,
        ...
        encoder=dict(
            ...
            encoder_layer=dict(
                ...
                embed_dims=_dim_,
                selfattention=dict(
                    # 公版为 TemporalSelfAttention；单帧下用 MSDeformAttn 做 BEV 空间自注意力
                    type="HorizonMultiScaleDeformableAttention",
                    embed_dims=_dim_,
                    num_points=vt_sa_num_points,
                    num_levels=1,
                    grid_align_num=4,
                    batch_first=True,
                    # reduce_align_num=8,
                ...
                crossattention=dict(
                    ...
                    deformable_attention=dict(
                        ...
                        embed_dims=_dim_,
                        num_points=vt_ca_num_points,
                        num_levels=_num_levels_,
                        grid_align_num=64,
                        feats_size=_vt_feats_size_,
                    ),
                    embed_dims=_dim_,
                ),
                dropout=0.1,
