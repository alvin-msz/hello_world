...
from hat.utils.aibenchmark_bpu.bev_process_bpu import preprocess_data, process_occ_data, process_occ_metric

from horizon_plugin_pytorch.quantization import get_qconfig
from horizon_plugin_pytorch.quantization.qconfig_setter import *  # noqa: F403
...
task_name = "flashocc_henet_lss_occ3d_nuscenes_rmbn"
num_classes = 18
...
data_rootdir = "/data/nuscenes/packed_new/"
...
def val_update_metric_func(metrics, batch, model_outs):
    gt_semantics = batch["gt_occ_info"]["voxel_semantics"][0].squeeze()  # (Dx, Dy, Dz)
    mask_camera = batch["gt_occ_info"]["mask_camera"][0].squeeze()  # (Dx, Dy, Dz)
...
calibration_val_callback["val_on_train_end"] = True
...
qtemplates=[  # 2. 模板，仅关注 dtype，按次序生效，因此模板的顺序很重要
    # 2.1. 基础配置部分
    ModuleNameTemplate({"": qint8}),  # 全局 feat int8
    ModuleNameTemplate(int16_qconfig, freeze=True,),
    ...
    model_convert_pipeline=dict(
        ...
        converters=[
            dict(
                type="LoadCheckpoint",
                checkpoint_path=os.path.join(
                    ckpt_dir, "float-checkpoint-epoch-0149-e91bab94.pth.tar"
                ),
            ),
        ],
    ),
    ...
    model_convert_pipeline=dict(
        ...
        converters=[
            ...
            dict(
                type="LoadCheckpoint",
                checkpoint_path=os.path.join(
                    ckpt_dir, "qat-checkpoint-epoch-0022-92d83ed2.pth.tar"
                ),
                ignore_extra=True,
            ),
        ],
...
# deploy_inputs = {
#     'img': torch.randn((6, 3, data_shape[1], data_shape[2])),
#     'points': [torch.randn(num_points, 128, 128, 2), torch.randn(num_points, 128, 128, 2)]
# }
...
compile_cfg = dict(
    ...
    hbm=os.path.join(compile_dir, "model_j6b.hbm"),
    layer_details=True,
