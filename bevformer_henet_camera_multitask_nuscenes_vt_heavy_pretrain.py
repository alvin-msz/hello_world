...
# VT 加强版：相对原 SingleBevFormer 轻量配置，约牺牲 ~50% 吞吐换精度
# （encoder 层数 / pillar 采样 / deformable 采样点向公版 tiny 对齐，仍保持单帧）
...
bev_sparse_rate = 0.4
# ViewTransformer 精度向公版 BEVFormer-tiny 对齐的关键超参（相对原 1/2/2 加强）
# 公版 tiny: encoder=3, pillar=4, SCA.num_points=8, TemporalSA + queue=3
# 本配置仍用 Single*（单帧），用更深 encoder + 更密采样换指标，避免改时序数据管线
vt_num_layers = 2
vt_num_points_in_pillar = 4
vt_sa_num_points = 4
vt_ca_num_points = 8
# 略提高 virtual BEV 密度（原 64x80≈31%），接近 bev_sparse_rate=0.4
vt_virtual_bev_h = 80
vt_virtual_bev_w = 96
...
camera_network = dict(
    ...
    view_transformer=dict(
        ...
        embed_dims=_dim_,
        queue_length=1,  # 保持单帧；若切公版时序需改 BevFormerViewTransformer + dataset num_seq
        in_indices=(-1,),
        single_bev=True,
        ...
        encoder=dict(
            ...
            encoder_layer=dict(
                ...
                embed_dims=_dim_,
                selfattention=dict(
                    # 公版为 HorizonTemporalSelfAttention；单帧下用 MSDeformAttn 做 BEV 空间自注意力
                ...
                crossattention=dict(
                    ...
                    deformable_attention=dict(
                        ...
                        grid_align_num=64,
                        feats_size=[[30, 16]],  # input_size(512,960) / stride32
                    ),
                    embed_dims=_dim_,
                ),
                dropout=0.1,
