"""Lidar-only PointPillar + BEVFormerDetDecoder (384-dim ablation).

Based on ``bevformer_pointpillar_nuscenes.py`` / CenterPoint PointPillar backbone.
Difference vs the baseline BevFormer-head config:

- ``embed_dims=384`` with identity BEV proj (no 384→256 squeeze into attention)

Training recipe matches the baseline (Cosine + differential LR + AMP, lidar
geometric augs). No CBGS / DataBaseSampler.
"""
...
import lidar_bevformer_det_adapter_opt  # noqa: F401
from lidar_bevformer_det_adapter_opt import remap_lidar_centerpoint_state_dict

VERSION = ConfigVersion.v2
training_step = os.environ.get("HAT_TRAINING_STEP", "float")

task_name = "bevformer_pointpillar_nuscenes_opt"
...
lidar_pretrain_ckpt = (
    "./tmp_models/bevformer_pointpillar_nuscenes_opt/"
    "float-checkpoint-epoch-0271-edf0674e.pth.tar"
)

# datadir settings — same BevSequence LMDB as baseline
...
# BEVFormerDetDecoder: keep SECOND 384ch into attention (no channel squeeze)
_dim_ = 384
bev_h_ = 128
bev_w_ = 128
...
_bevformer_decoder = dict(
    ...
    decoder=dict(
        ...
        decoder_layer=dict(
            ...
            # Must match BEVFormerDetDecoder.embed_dims (default is 256).
            embed_dims=_dim_,
            crossattention=dict(
...
bev_head = dict(
    ...
    out_channels=_dim_,
    # SECOND 384ch -> decoder 384ch, no compression
    use_identity_proj=True,
    use_fuse_proj=False,
...
# Lidar geometric augs only (no CBGS / DataBaseSampler).
train_dataset = dict(
        ...
        converters=[
            dict(
                ...
                checkpoint_path=os.path.join(
                    ckpt_dir, "float-checkpoint-epoch-0265-41b8e3a4.pth.tar"
                ),
                allow_miss=True,
                verbose=True,
            ),
