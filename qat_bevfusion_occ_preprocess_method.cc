...
#include "method/qat_bevfusion_occ_preprocess_method.h"
...
DEFINE_AND_REGISTER_METHOD(QATBevFusionOccPreProcessMethod);

namespace {

constexpr int32_t kExpectedCameraAuxInputs = 4;
constexpr int32_t kPointsInputIndex = 4;

}  // namespace

bool QATBevFusionOccPreProcessMethod::IsDirectory(const std::string &path) {
  struct stat st {};
...
std::string QATBevFusionOccPreProcessMethod::JoinPath(const std::string &dir,
...
bool QATBevFusionOccPreProcessMethod::TryLoadExactAlignedBin(
...
int32_t QATBevFusionOccPreProcessMethod::LoadFloatInputFromBin(
...
int32_t QATBevFusionOccPreProcessMethod::LoadAuxiliaryInput(
    hbDNNTensor *tensor, const std::string &bin_path, uint64_t *preprocess_us) {
...
int32_t QATBevFusionOccPreProcessMethod::LoadPointsInput(
    hbDNNTensor *tensor, const std::string &bin_path, uint64_t *preprocess_us) {
  if (TryLoadExactAlignedBin(tensor, bin_path)) {
    return 0;
  }
...
int32_t QATBevFusionOccPreProcessMethod::InitFromJsonString(
    const std::string &config) {
  VLOG(EXAMPLE_DEBUG) << "QATBevFusionOccPreProcessMethod Json string:"
                      << config.data();
  ...
  if (static_cast<int>(auxiliary_bin_names_.size()) != kExpectedCameraAuxInputs) {
    VLOG(EXAMPLE_SYSTEM)
        ...
        << " entries (img, post_rot, bda, sensor2ego), got "
...
int32_t QATBevFusionOccPreProcessMethod::DoProcess(std::string path,
                                                   int32_t input_count,
                                                   ImageTensor *image_tensor) {
  if (input_count != kPointsInputIndex + 1) {
    VLOG(EXAMPLE_SYSTEM) << "BEVFusion OCC expects " << (kPointsInputIndex + 1)
