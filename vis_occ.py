...
occ_class_names = [
    'undefined', 'barrier', 'bicycle', 'bus', 'car', 'cons. veh',
    'motorcycle', 'pedestrian', 'traffic cone', 'trailer', 'truck',
    'drive. surf.', 'other flat', 'sidewalk', 'terrain', 'manmade', 'vegetation'
]
...
def render_legend_on_image(canvas, class_names, class_colors, anchor_x=None, anchor_y=10,
                           swatch_w=18, swatch_h=18, row_gap=4, font_scale=0.45, thickness=1):
    num_classes = len(class_names)

    max_text_w = 0
    for name in class_names:
        (tw, _), _ = cv2.getTextSize(name, cv2.FONT_HERSHEY_SIMPLEX, font_scale, thickness)
        max_text_w = max(max_text_w, tw)

    block_w = swatch_w + 6 + max_text_w + 12  # padding
    block_h = num_classes * (swatch_h + row_gap) + row_gap

    if anchor_x is None:
        anchor_x = canvas.shape[1] - block_w - 8

    x1, y1 = anchor_x - 4, anchor_y - 4
    x2, y2 = anchor_x + block_w, anchor_y + block_h
    x1, y1 = max(x1, 0), max(y1, 0)
    x2, y2 = min(x2, canvas.shape[1]), min(y2, canvas.shape[0])
    overlay = canvas[y1:y2, x1:x2].copy()
    white_bg = np.full_like(overlay, 255)
    cv2.addWeighted(white_bg, 0.65, overlay, 0.35, 0, overlay)
    canvas[y1:y2, x1:x2] = overlay

    cur_y = anchor_y
    for idx, name in enumerate(class_names):
        bgr = class_colors[idx][:3][::-1].astype(np.uint8).tolist()  # RGBA->BGR
        cv2.rectangle(canvas,
                      (anchor_x, cur_y),
                      (anchor_x + swatch_w, cur_y + swatch_h),
                      bgr, -1)
        cv2.rectangle(canvas,
                      (anchor_x, cur_y),
                      (anchor_x + swatch_w, cur_y + swatch_h),
...
def parse_args():
    ...
    parser.add_argument('--draw-gt', action='store_true', default=False)
    parser.add_argument(
        '--root_path',
        type=str,
...
def main():
    ...
    info_path = \
        args.root_path + '/data/bevdetv2-nuscenes_infos_%s.pkl' % args.version
    ...
    for cnt, info in enumerate(
        ...
        #     # in case we are visualizing gt labels
        #     pred_occ_path = os.path.join(results_dir, scene_name, sample_token, 'label.npz')
        ...
        # if args.draw_gt:
            ...
            vis = show_occ(torch.from_numpy(voxel_label), torch.from_numpy(voxel_show), voxel_size=voxel_size, vis=vis,
                           offset=[0, voxel_label.shape[0] * voxel_size[0] * 1.2 * 0, 0])
        ...
        occ_canvas = np.ascontiguousarray(occ_canvas)

        render_legend_on_image(occ_canvas, occ_class_names, colormap_to_colors)
